...
const Overview = ({ dashboardData, loading, error }) => {
  ...
  // Handle the case where dashboardData is undefined or null
  const stats = dashboardData?.stats || { totalVehicles: 0, totalDrivers: 0, revenue: 0, occupancyRate: 0 };
  const bookings = dashboardData?.bookings || [];
  
  // If we have an error, we'll show it in DashboardLayout, so we don't need to handle it here
  // This allows the Overview component to render even with errors
        ...
        <div className="p-6">
          ...
          {sortedBookings.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400 mt-2">Add vehicles and drivers first to enable bookings.</p>
            </div>
          )}
        </div>
      </div>
